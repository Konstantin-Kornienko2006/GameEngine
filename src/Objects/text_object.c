...
FontCache *TextFontFind(char *path)
{
    ...
    FontCache *buff_fonts = engine.DataR.e_var_fonts;
...
void TextImageMakeTexture(GameObject2D *go, TextData *tData, BluePrintDescriptor *descriptor){
    ...
    if(engine.DataR.e_var_num_fonts + 1 >= MAX_FONTS)
    {
    ...
    char *some_file_path = (char *)tData->font.fontpath;
    ...
    if(font_len > 0)
    {
        temp_tex = TextFontFind((char *)tData->font.fontpath);
        ...
        if(temp_tex != NULL)
        {
            ...
            BuffersCreate(vertBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &go->graphObj.shapes[0].vParam.buffer, ENGINE_BUFFER_ALLOCATE_VERTEX);
    ...
    else if(engine.DataR.define_font_loaded)
    {
    ...
    fonts[engine.DataR.e_var_num_fonts].info = (stbtt_fontinfo *) AllocateMemory(1, sizeof(stbtt_fontinfo));
    fonts[engine.DataR.e_var_num_fonts].cdata = (stbtt_bakedchar *) AllocateMemory(1106, sizeof(stbtt_fontinfo));

    tData->font.cdata = fonts[engine.DataR.e_var_num_fonts].cdata;
    tData->font.info = fonts[engine.DataR.e_var_num_fonts].info;
    ...
    if(font_len > 0)
    {
        ...
        if(file != NULL)
        {
            ...
            stbtt_InitFont(fonts[engine.DataR.e_var_num_fonts].info , ttf_buffer, stbtt_GetFontOffsetForIndex(ttf_buffer,0));
            stbtt_BakeFontBitmap(ttf_buffer, 0, 32.0, temp_bitmap, tData->font.fontWidth, tData->font.fontHeight, 0, 1106, fonts[engine.DataR.e_var_num_fonts].cdata); // no guarantee this fits!
        }else
        {
            stbtt_InitFont(fonts[engine.DataR.e_var_num_fonts].info , &_binary_fonts_RobotoBlack_ttf_start, stbtt_GetFontOffsetForIndex(&_binary_fonts_RobotoBlack_ttf_start,0));
            stbtt_BakeFontBitmap(&_binary_fonts_RobotoBlack_ttf_start, 0, 32.0, temp_bitmap, tData->font.fontWidth, tData->font.fontHeight, 0, 1106, fonts[engine.DataR.e_var_num_fonts].cdata); // no guarantee this fits!
        }
    }else{
        stbtt_InitFont(fonts[engine.DataR.e_var_num_fonts].info , &_binary_fonts_RobotoBlack_ttf_start, stbtt_GetFontOffsetForIndex(&_binary_fonts_RobotoBlack_ttf_start,0));
        stbtt_BakeFontBitmap(&_binary_fonts_RobotoBlack_ttf_start, 0, 32.0, temp_bitmap, tData->font.fontWidth, tData->font.fontHeight, 0, 1106, fonts[engine.DataR.e_var_num_fonts].cdata); // no guarantee this fits!
    ...
    //--------------------
    //Создаем буфер вершин для плоскости
    BuffersCreate(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &go->graphObj.shapes[0].vParam.buffer, ENGINE_BUFFER_ALLOCATE_VERTEX);
    ...
    fonts[engine.DataR.e_var_num_fonts].texture = AllocateMemory(1, sizeof(Texture2D));

    Texture2D *texture = fonts[engine.DataR.e_var_num_fonts].texture;

    texture->flags |= ENGINE_TEXTURE2D_IS_FONT;

    TextureCreateImage(tData->font.fontWidth, tData->font.fontHeight, 1,VK_FORMAT_R8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0, texture);
    ...
    ToolsCopyBufferToImage(stagingBuffer.buffer, texture->image, tData->font.fontWidth, tData->font.fontHeight);
    ...
    descriptor->textures = fonts[engine.DataR.e_var_num_fonts].texture;

    engine.DataR.e_var_num_fonts ++;

}

void TextObjectMakeLastText(TextObject *to){
    TextImageSetText(to->textData.text, (GameObject2D *)to, &to->textData);
}
    ...
    for(int i=0; i < to->go.graphObj.gItems.num_shader_packs;i++)
    {
        ...
        if(pack->render_point == engine.current_render)
        {
            ...
            for(int j=0; j < pack->num_pipelines; j++){
                ...
                vkCmdBindVertexBuffers(command, 0, 1, (const VkBuffer *)&to->go.graphObj.shapes[settings->vert_indx].vParam.buffer.buffer, &offsets);
...
void TextDataSetFontPath(TextData* tData, const char* path){
    ...
    memcpy((void *)tData->font.fontpath, (void *)path, len);
}
...
void TextImageClearText(GameObject2D* go, TextData *tData)
{
    ...
    vkMapMemory(device->e_device, go->graphObj.shapes[0].vParam.buffer.memory, 0, VK_WHOLE_SIZE, 0, (void **)&mapped);

    memset(mapped, 0, BUFFER_SIZE * 4 * sizeof(Vertex2D));

    vkUnmapMemory(device->e_device, go->graphObj.shapes[0].vParam.buffer.memory);
    mapped = NULL;
}
...
void TextImageSetText(const uint32_t* text, GameObject2D* go, TextData *tData){
    ...
    len = ToolsStr32BitLength((uint32_t *)text);
    ...
    float mulX = tData->font.fontSize / engine.width / fontResizer;
    float mulY = tData->font.fontSize / engine.height / fontResizer;
...
void TextObjectSetColor(TextObject* to, vec3 color){
    
    TextDataSetTextColor(&to->textData, color);
...
void TextObjectInit(TextObject* to, int fontSize, const char* fontPath)
{
    ...
    GameObject2DInit((GameObject2D *)to);

    GameObjectSetDrawFunc((GameObject *)to, (void *)TextObjectDrawDefault);

    //Загружаем шрифт и настраеваем его на работу
    TextDataInit(&to->textData, fontSize, (char *)fontPath);
...
void TextObjectInitDefault(TextObject *to, int fontSize, const char* fontPath, DrawParam *dParam)
{
    ...
    GameObject2DInitDraw((GameObject2D *)to);
}
